...
from app.forms import LoginForms, RegisterationForm, EditProfileForm, EmptyForm, PostForm
...
def before_request():
    if current_user.is_authenticated:
        ...
        db.session.commit() # no need for db.session.add() as the user added to session by invoking current_user
